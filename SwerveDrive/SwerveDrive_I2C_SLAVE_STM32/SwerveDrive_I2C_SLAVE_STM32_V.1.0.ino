...
void receiveEvent(int dataSize) {
  ...
  if (separatorIndex != -1) {
    speed = receivedString.substring(0, separatorIndex).toInt();
    degree = receivedString.substring(separatorIndex + 1).toInt();
...
void setup() {
  Serial.begin(115200);
  Serial.println("STM32 Slave Started - Address: " + String(SLAVE_ADDR));
  ...
  Serial.println("STM32 Setup Completed");
}
